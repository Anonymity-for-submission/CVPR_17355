...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.linear_fractal = nn.Linear(128 * block.expansion, num_classes)
        self.relu = nn.ReLU()
    ...
    def forward(self, x, iffractal=False):
        out = self.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        if iffractal:
...
def ResNet18(class_no=10):
    return ResNet(BasicBlock, [2, 2, 2, 2], class_no)


def ResNet34(class_no=100):
    return ResNet(BasicBlock, [3, 4, 6, 3], class_no)
